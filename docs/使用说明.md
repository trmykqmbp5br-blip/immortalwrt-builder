...
5. Get date for ccache key
6. Configure feeds        # diy-part1.sh（config-manifest → generate-provides → ln -sf → feeds update/install）
7. Install feeds          # feeds install -a（diy-part1.sh 已内置，此步冗余但无害）
8. Apply custom config    # diy-part2.sh（P0 防御 + strict config-manifest 加载）
9. Restore ccache         # actions/cache@v5（自动恢复 + job 结束自动保存）
10. Download sources       # make download -j$(nproc)
11. Save dl cache          # 成功/失败分别保存
12. Compile firmware       # make -j$(nproc) V=s
13. ccache stats           # ccache -s + 超 6GB 清理
14. Save ccache on failure # 构建失败时保存失败缓存
15. Verify binary inject   # 检查 binary-manifest.json 的 skipped 包
16. Release firmware       # 上传 img.gz + sysupgrade.bin.gz，保留 3 个
```
...
**关键**：OpenWrt 的 `rules.mk` 会 export `CCACHE_DIR`，覆盖环境变量。`.config` 中 `CONFIG_CCACHE_DIR` 为空时 → export `CCACHE_DIR=""` → ccache 使用 XDG 默认 `~/.cache/ccache`，与 GitHub cache action 路径不匹配。diy-part2.sh 在 make defconfig 之前注入 `CONFIG_CCACHE_DIR="$HOME/.ccache"` 到 .config，`$HOME` 在 GitHub Actions runner 上展开为 `/home/runner`，make defconfig 保留已设值，确保路径与 cache action 一致（注意：build.yml 中 actions/cache 的 path 必须硬编码 `/home/runner/.ccache`，不可用 `~`，否则缓存版本计算不一致导致旧缓存失效）。
...
| `diy-part2.sh` | 主编排器：P0 环境变量检查 → kernel-config/runtime → make defconfig → strict config-manifest + apply_manifest → prepare-binary → 校验 |
...
修复：diy-part2.sh 在 make defconfig 之前注入 `CONFIG_CCACHE_DIR="$HOME/.ccache"` 到 .config，`$HOME` 在 runner 上展开为 `/home/runner`，make defconfig 保留已设值，确保 OpenWrt 内部 export 的路径与 cache action 一致。
### 为什么需要 PROVIDES 虚拟包？
