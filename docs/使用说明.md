...
新增 BINARY 包后，在 `scripts/binary-packages.sh` 的 `BINARY_PACKAGES_FLAT` 中追加包名即可，
`diy-part1.sh` 中的 `generate-provides.sh` 会自动重新生成 PROVIDES Makefile。

**不需要**手动编辑 `package/custom-provides/Makefile`，下次构建时 diy-part1.sh 会自动处理。
