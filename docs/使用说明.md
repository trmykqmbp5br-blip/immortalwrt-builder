...
7. Configure feeds        # diy-part1.sh（config-manifest → generate-provides → ln -sf → feeds update/install → @BROKEN 替换 BINARY 包）
8. Apply custom config    # diy-part2.sh（P0 防御 → kernel/runtime → make defconfig → kconfig-tool 启用 CCACHE+SOURCE+PROVIDES）
9. Restore ccache         # actions/cache@v5（自动恢复 + job 结束自动保存）
10. Download sources       # make download -j$(nproc)
11. Save dl cache          # 成功/失败分别保存（带日期 key）
12. Compile firmware       # chown/chmod ccache → ccache -z/M/s → make -j2 V=1 CONFIG_CCACHE=y CONFIG_CCACHE_DIR=...
...
BINARY 包被 @BROKEN 标记后，`make defconfig` 自动设为 `n`。defconfig 之后用 `kconfig-tool` 启用 CCACHE 和 PROVIDES 虚拟包：
...
编译命令通过 `CONFIG_CCACHE=y CONFIG_CCACHE_DIR="/home/runner/.ccache"` 参数直接传给 make，
从源头上确保 OpenWrt 的 `rules.mk` export 正确的路径，与 cache action 保存的 `/home/runner/.ccache` 一致。

**编译步骤**中还做了：`chown/chmod` 确保写权限 + `ccache -z` 重置统计 + `ccache -M 5G` 限容 + `ccache -s` 打印状态。
...
| `diy-part2.sh` | 主编排器：P0 防御 → kconfig-tool 下载 → kernel/runtime 补丁 → make defconfig（@BROKEN 自动禁用 BINARY） → kconfig-tool 启用 CCACHE+SOURCE+PROVIDES → strict config-manifest + apply_manifest → prepare-binary → 校验 |
...
使用 Linux 内核官方的 `scripts/config` 脚本（重命名为 `kconfig-tool`，避免与 OpenWrt 的 `scripts/config/` 目录冲突）。
BINARY 包不再需要 `--disable`，因为 diy-part1.sh 已将其 Makefile 替换为 `DEPENDS:=@BROKEN`，`make defconfig` 自动设为 `n`。
apply_manifest 仅处理 EXCLUDE 和 SOURCE 包。同时自动启用 `CONFIG_PACKAGE_custom-binary-provides`（编译空包，PROVIDES 所有 BINARY 包），
编译期依赖系统认为 BINARY 包已存在，避免 `depends on` 导致的编译中断。

调用时机：必须在 make defconfig 之后执行，且之后绝不再执行 make defconfig。
...
diy-part1.sh（P0 防御：GITHUB_WORKSPACE 检查）
  → source config-manifest.sh + generate-provides.sh（动态生成 PROVIDES Makefile）
  → ln -sf 链接 custom-provides 到 openwrt 源码树
  → 添加第三方 feeds（kenzo/small）到 feeds.conf.default
  → ./scripts/feeds update -a / install -a
  → @BROKEN 替换所有 BINARY 包的 Makefile（defconfig 时自动 =n）

diy-part2.sh（P0 防御：GITHUB_WORKSPACE 检查 + strict config-manifest 加载）
  → chmod +x scripts/*.sh + 下载 Linux 官方 scripts/config 为 kconfig-tool
  → kernel-config/runtime patches（IA32_EMULATION + olddefconfig via kernel-config-patch.py）
  → make defconfig（@BROKEN 包自动 n，无需手动禁用）
  → ./scripts/kconfig-tool --enable CONFIG_CCACHE
  → ./scripts/kconfig-tool --set-str CONFIG_CCACHE_DIR "/home/runner/.ccache"
  → ./scripts/kconfig-tool --enable CONFIG_PACKAGE_custom-binary-provides
  → strict source config-manifest.sh + apply_manifest（仅处理 EXCLUDE + SOURCE）
  → prepare-binary.sh → IPK 排序 → x86 boot 修复 → 校验
  → prepare-binary.sh
...
修复：diy-part2.sh 在 defconfig 后通过 `./scripts/kconfig-tool --set-str CCACHE_DIR "/home/runner/.ccache"` 设置路径，编译命令通过 `CONFIG_CCACHE=y CONFIG_CCACHE_DIR="/home/runner/.ccache"` 参数强锁，确保路径始终与 cache action 一致。
### 为什么需要 PROVIDES 虚拟包？
