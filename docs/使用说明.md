...
6. Configure feeds        # diy-part1.sh（source config-manifest → generate-provides → ln -sf → feeds update/install）
7. Install feeds          # feeds install -a（diy-part1.sh 已内置，此步冗余但无害）
8. Apply custom config    # diy-part2.sh（含 P0 防御 + strict config-manifest 加载）
