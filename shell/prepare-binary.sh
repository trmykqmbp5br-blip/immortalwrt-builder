...
download_gh_release() {
    ...
    if [ -z "$dl_url" ]; then
        if echo "$resp" | grep -qi "rate limit"; then
            echo "    WARN: GitHub API rate limited! Set GH_TOKEN env to avoid this." >&2
        fi
        return 1
    fi
...
# 遍历 $WANTED 列表，下载 ipk/二进制到 files/。
# ipk 存入 files/etc/ipk-cache/，生成 uci-defaults 开机安装脚本。
# ================================================================
prepare_binary_packages() {
    ...
    local has_ipk=false
    local LOG_FILE="/etc/config/ipk-install.log"
    ...
    for pkg in $WANTED; do
        ...
        case "$method" in
            ...
            gh-bin)
                ...
                ;;
            store)
                echo "  $pkg (handled by prepare-store.sh)"
...
# prepare-binary.sh 自动生成，执行成功后自删
...
echo "[$(date)] Starting ipk install from $IPK_DIR ..." >> "$LOG"

all_ok=0
for ipk in "$IPK_DIR"/*.ipk; do
    [ -f "$ipk" ] || continue
    echo "  Installing: $(basename "$ipk")" >> "$LOG"
    opkg install "$ipk" --force-depends >> "$LOG" 2>&1 || all_ok=1
    if [ -f "$ipk" ]; then
        rm -f "$ipk" 2>/dev/null
    fi
done
...
if [ "$all_ok" -eq 0 ]; then
    echo "[$(date)] All ipk installed successfully" >> "$LOG"
else
    echo "[$(date)] Some ipk installs had errors (check above)" >> "$LOG"
fi
...
generate_manifest() {
    ...
    local total=0 downloaded=0 extracted=0 skipped=0 pending=0
    for pkg in $wanted; do
        : $((total++))
