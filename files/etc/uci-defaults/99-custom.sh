...
if [ "$count" -gt 1 ]; then
    ...
    if [ -n "$wanb_ifname" ]; then
        echo "配置第二 WAN (wanb): $wanb_ifname" >>$LOGFILE

        # 添加 WANB 接口（PPPoE 拨号）
        uci set network.wanb=interface
        uci set network.wanb.device="$wanb_ifname"
        uci set network.wanb.proto='pppoe'
        uci set network.wanb.username='13765301824'
        uci set network.wanb.password='849603'
        ...
        # 配置 mwan3（如果安装了 mwan3）
        if command -v mwan3 >/dev/null 2>&1; then
            echo "检测到 mwan3，配置负载均衡..." >>$LOGFILE
            ...
            uci set mwan3.wan.gateway='10.22.128.1'
